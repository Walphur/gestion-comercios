...
/// Sube la carta pública (gastronomía) para walqo.pro/carta.
pub fn push_menu_portal_snapshot(snapshot: serde_json::Value) -> Result<(), String> {
    let conn = open_exclusive().map_err(|e| format!("Base de datos: {e}"))?;
    let token = read_setting(&conn, "license_token")
        .map(|t| t.trim().to_string())
        .filter(|t| t.starts_with("GC1."));
    ...
    let url = format!("{}/v1/menu-portal/push", license_api_url());
    let client = reqwest::blocking::Client::builder()
        .timeout(std::time::Duration::from_secs(45))
        .build()
        .map_err(|e| format!("No se pudo iniciar la conexión: {e}"))?;
