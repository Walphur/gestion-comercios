...
import {
  checkPaywayPaymentStatus,
  createPaywayQrOrder,
  PAYWAY_QR_MIN_AMOUNT,
  type PaywayQrOrderResult,
...
interface Props {
  ...
  currency: string;
  description: string;
  onApproved: (info: { paymentId: string; intentionId?: string | null }) => void;
  onClose: () => void;
}

export default function PaywayQrModal({
  open,
  amount,
  currency,
  description,
  ...
  const [order, setOrder] = useState<PaywayQrOrderResult | null>(null);
  ...
  const startOrder = useCallback(async () => {
    ...
    if (amount < PAYWAY_QR_MIN_AMOUNT) {
      setError(
        `El monto es muy pequeño para Payway QR. Mínimo: ${formatMoney(PAYWAY_QR_MIN_AMOUNT, currency)}.`,
      );
      setBusy(false);
      return;
    }
    try {
      ...
      const created = await createPaywayQrOrder(amount, description, ref);
      setOrder(created);
      ...
      setStatusText(
        created.simulated
          ...
          : "Pedile al cliente que escanee con su banco o billetera (MODO, etc.).",
      );
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        const st = await checkPaywayPaymentStatus(order.payment_id, order.simulated);
        if (st.status === "approved") {
          clearInterval(id);
          onApproved({
            paymentId: order.payment_id,
            intentionId: order.intention_id,
          });
        } else {
          setStatusText(
            order.simulated
  ...
  return (
    <Modal open={open} title="Cobrar con Payway QR" onClose={onClose} wide>
      <div className="text-center">
        ...
        {qrImage && !busy && (
          <div className="mx-auto my-4 inline-block rounded-2xl border border-[var(--color-panel-border)] bg-white p-4">
            <img src={qrImage} alt="QR Payway" className="h-[280px] w-[280px]" />
          </div>
        )}
