...
/** Ícono WhatsApp compacto (barra de estado). */
export default function WhatsAppAssistButton({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={() =>
        void openVirtualAssist({
          businessName,
          userName: user?.display_name || user?.username || null,
        })
      }
      className={`inline-flex h-7 w-7 items-center justify-center rounded-lg text-[#25D366] transition hover:bg-[#25D366]/15 ${className}`}
